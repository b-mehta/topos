...
lemma downward_closed (S : sieve X) (f : over X) (Hf : f ∈ S) (g : Z ⟶ f.left) :
  over.mk (g ≫ f.hom) ∈ S :=
S.subs f Hf g

lemma downward_closed' (S : sieve X) (f : Y ⟶ X) (Hf : over.mk f ∈ S) (g : Z ⟶ Y) :
  over.mk (g ≫ f) ∈ S :=
S.subs (over.mk f) Hf g

instance : has_subset (sieve X) := ⟨λ S R, S.arrows ⊆ R.arrows⟩

@[ext] def ext : Π {R S : sieve X}, R.arrows = S.arrows → R = S
| ⟨Ra, _⟩ ⟨Sa, _⟩ rfl := rfl
...
{ le           := λ S R, S ⊆ R,
  ...
  le_antisymm  := begin intros S R p q, apply sieve.ext, apply set.subset.antisymm; assumption end,
...
@[simp] lemma mem_pullback (h : Y ⟶ X) {f : Z ⟶ Y} :
  over.mk f ∈ (pullback S h).arrows ↔ over.mk (f ≫ h) ∈ S := iff.rfl

@[simp] lemma mem_pullback2 (h : Y ⟶ X)  {f : over Y} :
  f ∈ (pullback S h).arrows ↔ over.mk (f.hom ≫ h) ∈ S := iff.rfl
...
begin apply top_unique, rintros g Hg, exact Hg end

lemma le_pullback_comp {R : sieve Y} {f : Y ⟶ X} :
  R ≤ pullback (comp R f) f :=
begin rintros g b, refine ⟨_, _, rfl⟩, simpa end
...
{ obj := λ Y, {g : Y.unop ⟶ X // over.mk g ∈ S},
  map := λ Y Z f g, subtype.mk (f.unop ≫ g.1)
  (begin
    cases g with g gS,
    apply sieve.subs S (over.mk g) gS f.unop,
  end) }
